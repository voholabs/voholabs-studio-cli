...
By default, `voholabs auth:login` uses the hosted auth server at `cli-auth.postiz.com`. If you want to self-host the OAuth2 device flow server, follow the guide in [`server/SERVER.md`](./server/SERVER.md).
...
```bash
export VOHOLABS_API_URL=https://your-custom-api.com
```
...
| Error | Solution |
|-------|----------|
| `Not authenticated` | Run `voholabs auth:login` or set `VOHOLABS_API_KEY` |
...
```bash
# Authentication
voholabs auth:login                                              # OAuth2 device flow
postiz auth:status                                             # Check auth
postiz auth:logout                                             # Remove credentials
export VOHOLABS_API_KEY=your_key                                 # Or use API key
