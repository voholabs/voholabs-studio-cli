...
**You MUST authenticate before running any Voholabs CLI command.** All commands will fail without valid credentials.
...
1. **OAuth2:** `voholabs auth:login`
2. **API Key:** `export VOHOLABS_API_KEY=your_api_key`
...
The fundamental pattern for using Voholabs CLI:
...
# 1. Authenticate
postiz auth:status
# If not authenticated: voholabs auth:login --client-id <id> --client-secret <secret>
...
**Option 2: API Key**
```bash
export VOHOLABS_API_KEY=your_api_key_here
```

**Optional custom API URL:**
```bash
export VOHOLABS_API_URL=https://custom-api-url.com
```
...
1. **Not authenticated** - Run `voholabs auth:login` or `export VOHOLABS_API_KEY=key` before using CLI
...
voholabs auth:login                                              # OAuth2 device flow login
postiz auth:logout                                             # Remove credentials
export VOHOLABS_API_KEY=key                                      # Or use API key
